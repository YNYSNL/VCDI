...
class DataProcess(object):
    ...
    def neighbors_process(self, sdc_id, timestep, tracks):
        ...
        for neighbor in sorted_neighbors:
            ...
            # print('track_id',tracks[neighbor_id].id)
    ...
    def mass_model(self, init_state, ax, ay):
        x, y, h, vx, vy = init_state
        track = [[x, y, h, vx, vy, ax[0], ay[0]]]
        for i in range(len(ax) - 1):
            ...
            track.append([x, y, heading, vx, vy, ax[i+1], ay[i+1]])
    ...
    def normalize_data(self, ego, neighbors, map_lanes, map_crosswalks, ref_line, ground_truth, viz=True):
        ...
        ego[:, :7] = agent_norm(ego, center, angle)
        ground_truth[0] = agent_norm(ground_truth[0], center, angle)

        for i in range(neighbors.shape[0]):
            if neighbors[i, -1, 0] != 0:
                neighbors[i, :, :7] = agent_norm(neighbors[i], center, angle, impute=True)
        ...
        # visulization
        if viz:
            rect = plt.Rectangle((ego[-1, 0] - ego[-1, 7] / 2, ego[-1, 1] - ego[-1, 8] / 2), ego[-1, 7], ego[-1, 8],
            ...
            for i in range(neighbors.shape[0]):
                if neighbors[i, -1, 0] != 0:
                    rect = plt.Rectangle(
                        (neighbors[i, -1, 0] - neighbors[i, -1, 7] / 2, neighbors[i, -1, 1] - neighbors[i, -1, 8] / 2),
                        neighbors[i, -1, 7], neighbors[i, -1, 8], linewidth=2, color='m', alpha=0.6, zorder=3,
