...
class LaneEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        lane_attr = self_type + left_type + right_type + \
            traffic_light + stop_point + interpolating + stop_sign
...
class CrosswalkEncoder(nn.Module):
    def __init__(self):
        ...
        self.point_net = nn.Sequential(nn.Linear(3, 64), nn.ReLU(), nn.Linear(
            64, 128), nn.ReLU(), nn.Linear(128, 256), nn.ReLU())
...
class CrossTransformer(nn.Module):
    def __init__(self):
        ...
        self.cross_attention = nn.MultiheadAttention(
            256, 8, 0.1, batch_first=True)
        self.transformer = nn.Sequential(nn.LayerNorm(256), nn.Linear(
            256, 1024), nn.ReLU(), nn.Dropout(0.1), nn.Linear(1024, 256), nn.LayerNorm(256))
...
class MultiModalTransformer(nn.Module):
    def __init__(self, modes=3, output_dim=256):
        ...
        self.modes = modes
        self.attention = nn.ModuleList([nn.MultiheadAttention(
            256, 4, 0.1, batch_first=True) for _ in range(modes)])
        self.ffn = nn.Sequential(nn.LayerNorm(256), nn.Linear(256, 1024), nn.ReLU(
        ), nn.Dropout(0.1), nn.Linear(1024, output_dim), nn.LayerNorm(output_dim))

    def forward(self, query, key, value, mask=None):
        attention_output = []
        for i in range(self.modes):
            attention_output.append(self.attention[i](
                query, key, value, key_padding_mask=mask)[0])
...
class EgoFutureTransformer(nn.Module):
    def __init__(self):
        super(EgoFutureTransformer, self).__init__()
        self.agent_interaction_ego_future = CrossTransformer()

    def forward(self, agent_interaction, ego_future, mask=None):
        query = agent_interaction
        key = torch.cat([ego_future, agent_interaction], dim=1)
        output = self.agent_interaction_ego_future(query, key, key, mask)
...
class Agent2Map(nn.Module):
    ...
    def forward(self, actor, lanes, crosswalks, mask):
        ...
        lanes_actor = [self.lane_attention(
            query, lanes[:, i], lanes[:, i]) for i in range(lanes.shape[1])]
        crosswalks_actor = [self.crosswalk_attention(
            query, crosswalks[:, i], crosswalks[:, i]) for i in range(crosswalks.shape[1])]
        map_actor = torch.cat(lanes_actor+crosswalks_actor, dim=1)
        output = self.map_attention(
            query, map_actor, map_actor, mask).squeeze(2)
...
class AgentDecoder(nn.Module):
    def __init__(self, future_steps, model_type='VCDI'):
        super(AgentDecoder, self).__init__()
        self._future_steps = future_steps
        self.model_type = model_type
        if self.model_type in {'VCDI', 'Gaussian'}:
            emb_dim = 768
        elif self.model_type in {'DIPP'}:
            emb_dim = 512
        self.decode = nn.Sequential(nn.Dropout(0.1), nn.Linear(
            emb_dim, 256), nn.ELU(), nn.Linear(256, future_steps*3))
    ...
    def forward(self, agent_map, agent_agent, current_state, ego_future_related=None):
        if self.model_type in {'VCDI', 'Gaussian'}:
            feature = torch.cat([agent_map, agent_agent.unsqueeze(1).repeat(
                1, 3, 1, 1), ego_future_related.unsqueeze(1).repeat(1, 3, 1, 1)], dim=-1)
        elif self.model_type in {'DIPP'}:
            feature = torch.cat([agent_map, agent_agent.unsqueeze(1).repeat(
                1, 3, 1, 1)], dim=-1)
        decoded = self.decode(feature).view(-1, 3, 10, self._future_steps, 3)
        trajs = torch.stack([self.transform(decoded[:, i, j], current_state[:, j])
                            for i in range(3) for j in range(10)], dim=1)
...
class AVDecoder(nn.Module):
    def __init__(self, future_steps=50, feature_len=9, model_type='VCDI'):
        super(AVDecoder, self).__init__()
        self._future_steps = future_steps
        self.model_type = model_type
        self.control = nn.Sequential(nn.Dropout(0.1), nn.Linear(
            512, 256), nn.ELU(), nn.Linear(256, future_steps*2))
        if self.model_type in {'VCDI', 'DIPP'}:
            self.cost = nn.Sequential(nn.Linear(1, 128), nn.ReLU(
            ), nn.Linear(128, feature_len), nn.Softmax(dim=-1))
        elif self.model_type in {'Gaussian'}:
            self.cost = nn.Sequential(GaussianLayer(1, 128), nn.ReLU(
            ), nn.Linear(128, feature_len), nn.Softmax(dim=-1))
    ...
    def forward(self, agent_map, agent_agent):
        feature = torch.cat(
            [agent_map, agent_agent.unsqueeze(1).repeat(1, 3, 1)], dim=-1)
        actions = self.control(feature).view(-1, 3, self._future_steps, 2)
        if self.model_type in {'Gaussian'}:
...
class Score(nn.Module):
    def __init__(self):
        ...
        self.reduce = nn.Sequential(nn.Dropout(
            0.1), nn.Linear(512, 256), nn.ELU())
        self.decode = nn.Sequential(nn.Dropout(0.1), nn.Linear(
            512, 128), nn.ELU(), nn.Linear(128, 1))

    def forward(self, map_feature, agent_agent, agent_map):
        # pooling
        map_feature = map_feature.view(
            map_feature.shape[0], -1, map_feature.shape[-1])
        ...
        feature = torch.cat([feature.unsqueeze(1).repeat(
            1, 3, 1), agent_map.detach()], dim=-1)
...
class Predictor(nn.Module):
    def __init__(self, future_steps, model_type='VCDI'):
        ...
        self.ego_agent = EgoFutureTransformer()

        # decode layers
        self.plan = AVDecoder(self._future_steps, model_type=self.model_type)
        self.predict = AgentDecoder(
            self._future_steps, model_type=self.model_type)
        self.score = Score()

    def forward(self, ego, neighbors, map_lanes, map_crosswalks, future_action):
        # actors
        ego_actor = self.vehicle_net(ego)
        av_future_action = self.av_action_net(future_action).unsqueeze(1)
        vehicles = torch.stack([self.vehicle_net(neighbors[:, i])
                               for i in range(10)], dim=1)
        pedestrians = torch.stack([self.pedestrian_net(
            neighbors[:, i]) for i in range(10)], dim=1)
        cyclists = torch.stack([self.cyclist_net(neighbors[:, i])
                               for i in range(10)], dim=1)
        ...
        actor_mask = torch.eq(
            torch.cat([ego.unsqueeze(1), neighbors[..., :-1]], dim=1), 0)[:, :, -1, -1]
        ...
        if self.model_type in {'VCDI', 'Gaussian'}:
            action_mask = torch.full(
                (actor_mask.shape[0], 1), True, dtype=torch.bool).to(actor_mask.device)
            actor_action_mask = torch.cat([actor_mask, action_mask], dim=1)
            ego_agent = self.ego_agent(
                agent_agent, av_future_action, actor_action_mask)

            # plan + prediction
            plans, cost_function_weights = self.plan(
                agent_map[:, :, 0], agent_agent[:, 0])

            predictions = self.predict(
                agent_map[:, :, 1:], agent_agent[:, 1:], neighbors[:, :, -1], ego_agent[:, 1:])
