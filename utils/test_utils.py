...
def map_process(map_feature, map_type):
    if map_type == 'lane':
        ...
        if map_feature.type == 1:
            plt.plot(polyline[:, 0], polyline[:, 1], 'lightgray', linestyle='dashed', linewidth=2)
        elif map_feature.type == 2:
            plt.plot(polyline[:, 0], polyline[:, 1], 'lightgray', linestyle='solid', linewidth=2)
            ...
            plt.plot(polyline[:, 0], polyline[:, 1], 'lightgray', linestyle='dotted', linewidth=2)
        else:
            plt.plot(polyline[:, 0], polyline[:, 1], 'lightgray', linewidth=1)
        ...
        if map_feature.type == 1:
            plt.plot(polyline[:, 0], polyline[:, 1], 'darkgray', linewidth=2)
        elif map_feature.type == 2:
            plt.plot(polyline[:, 0], polyline[:, 1], 'darkgray', linewidth=1)
        else:
            plt.plot(polyline[:, 0], polyline[:, 1], 'darkgray', linewidth=1)
        ...
        plt.gca().add_patch(plt.Circle(position, 2, color='w'))

        return position

    elif map_type == 'crosswalk':
        polyline = polygon_completion(map_feature.polygon).astype(np.float32)
        plt.plot(polyline[:, 0], polyline[:, 1], 'sandybrown', linewidth=2, linestyle='solid')

        return polyline

    elif map_type == 'speed_bump':
        polyline = polygon_completion(map_feature.polygon).astype(np.float32)
        plt.plot(polyline[:, 0], polyline[:, 1], 'xkcd:orange', linewidth=0)
...
def check_prediction(trajs, gt):
    ...
    for i in range(10):
        if mask[i, 0, 0]:
            ...
            FDE_1s.append(error[-41])
            FDE_3s.append(error[-21])
            FDE_5s.append(error[-1])

    return np.mean(ADE), np.mean(FDE_1s),np.mean(FDE_3s),np.mean(FDE_5s)