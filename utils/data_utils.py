...
def imputer(traj):
    x, y, v_x, v_y, theta = traj[:, 0], traj[:, 1], traj[:, 3], traj[:, 4], traj[:, 2]
    a_x, a_y = traj[:, 5], traj[:, 6]

    if np.any(x==0):
        for i in reversed(range(traj.shape[0]-1)):
            if x[i] == 0:
                v_x[i] = v_x[i+1]
                v_y[i] = v_y[i+1]
                x[i] = x[i+1] - v_x[i]*0.1
                ...
                a_x[i] = a_x[i + 1]
                a_y[i] = a_y[i + 1]
        return np.column_stack((x, y, theta, v_x, v_y, a_x, a_y))
    else:
        return np.column_stack((x, y, theta, v_x, v_y, a_x, a_y))
...
def agent_norm(traj, center, angle, impute=False):
    ...
    if traj.shape[-1] > 3:
        ...
        if traj.shape[-1] > 5:
            acceleration_x = traj[:, 5] * np.cos(angle) + traj[:, 6] * np.sin(angle)
            acceleration_x[traj[:, 5] == 0] = 0
            acceleration_y = traj[:, 6] * np.cos(angle) - traj[:, 5] * np.sin(angle)
            acceleration_y[traj[:, 6] == 0] = 0

            return np.column_stack((line_rotate, heading, velocity_x, velocity_y, acceleration_x, acceleration_y))
        else:
            return np.column_stack((line_rotate, heading, velocity_x, velocity_y))
    else:
        return np.column_stack((line_rotate, heading))
